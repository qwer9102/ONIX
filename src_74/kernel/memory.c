...
void free_pde()
{
    ...
    for(size_t didx = 2; didx < 1023; didx++)
    {
        ...
        //页目录项有内容,那么遍历对应的页表项
        page_entry_t *pte = (page_entry_t*)(PDE_MASK | (didx << 12));//找到对应的页表地址
        for(size_t tidx = 0; tidx < 1024; tidx++)
        {
            page_entry_t *entry = &pte[tidx];  //指向页表项
            if(!entry->present)
               { continue;}   //页表项没有内容
            
            assert(memory_map[entry->index] > 0);  //有映射
            put_page(PAGE(entry->index));                //entry->index是物理页号的索引
        }
...
int32 sys_brk(void *addr)
{
    ...
    if(old_brk > brk)
    {
        for(u32 page = brk;brk < old_brk; page += PAGE_SIZE)  //addr要求是页起始地址
        {
            unlink_page(brk);
        }
    }
