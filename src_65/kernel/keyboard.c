...
static bool extcode_state;   //k扩展码
...
//键盘直接从寄存器获取的值是按照第一套编码方式对应的数值,中断函数需要将其转换成对应的字母和数字    
//按下键盘和松开键盘总共执行了两次中断,按下键盘是范围是0-127(7F)(第8位是0),松开范围128-256是(第8位是1)
void keyboard_handler(int vector)
{
    assert(vector == 0x21);
    send_eoi(vector);            // 向中断控制器发送中断处理结束的信息
    u16 scancode = inb(KEYBOARD_DATA_PORT);   //从键盘读取按键信息    
    //LOGK("keyboard input is 0x%d",scancode);   
    u8 ext = 2;   //keymap索引对应的列,默认没有按下shift键

    //扩展码键位
    if(scancode == 0xe0)
    {
        extcode_state = true ;  //设置为扩展状态
        return;
    }
    //是扩展码
    if(extcode_state)
    {
        ext = 3;             //修改keymap为列3对应的值
        scancode |= 0xe000;  
        extcode_state = false;    
    }
    ...
    //通码非法,按下键盘的数值范围是0-0x7f,这里keymap按下键位数值只定义到0x5F
    if(makecode > KEY_PRINT_SCREEN)
    {
        return;
    }

    //是否是断码,按键抬起,断码不打印,直接return
    //是断码,表示松开
    ...
    //大写锁定同时按下shift是小写;大写没锁定按下shift,shift反转
    if(shift_state)
    {
        shift = !shift;
    }
